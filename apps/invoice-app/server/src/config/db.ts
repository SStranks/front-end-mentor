...
mongoose.connection.on('connecting', () => {
  console.log(`*** Connecting to MongoDB database: ${DB_DATABASE} @ ${DB_HOST}`);
});
...
mongoose.connection.on('reconnected', () => {
  console.log(`*** Reconnected to MongoDB database: ${DB_DATABASE} @ ${DB_HOST}`);
});

mongoose.connection.on('disconnected', () => {
  console.log(`*** Disconnected from MongoDB database: ${DB_DATABASE} @ ${DB_HOST}`);
});

mongoose.connection.on('error', (error) => {
  console.log(`*** MongoDB database error for: ${DB_DATABASE} @ ${DB_HOST}`, `*** Error: ${error}`);
});
