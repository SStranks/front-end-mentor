...
function DropdownList(props) {
  ...
  const renderListItems = listItems.map((item, i) => (
    <li className={styles.list__li} key={i} onChange={onRadioCheck}>
      <input type="radio" className={styles.list__radio} id={item} name={name} value={item} defaultChecked={!i} />
  ...
  return <ul className={`${styles.list} ${active ? '' : styles.hidden}`}>{renderListItems}</ul>;
}
