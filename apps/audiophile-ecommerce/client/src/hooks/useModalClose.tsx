...
function useModalClose(setModal: Param1, modalRef: Param2, openModalBtnRef: Param3): void {
  return useEffect(() => {
    const closeEscKey = (e: KeyboardEvent) => (e.key === 'Escape' ? setModal(false) : undefined);

    const clickOutsideModal = (e: MouseEvent) => {
      // If the click was on modal open/close button, ignore it - let the parent toggle state.
      if (typeof openModalBtnRef !== 'function' && openModalBtnRef?.current?.contains(e.target as HTMLElement)) return;
      if (typeof modalRef !== 'function' && !modalRef?.current?.contains(e.target as HTMLElement)) {
        setModal(false);
      }
    };
