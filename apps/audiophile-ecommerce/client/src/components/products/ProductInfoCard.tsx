...
type ElemProps = {
  ...
  productId: number;
  productImgs: { desktop: string; mobile: string; tablet: string };
  productTitle: string;
};

function ProductInfoCard(props: ElemProps): JSX.Element {
  const { productImgs, productTitle, productDetails, newProduct, productCategory, productId } = props;

  return (
    <article className={styles['card']} aria-labelledby={productTitle}>
      <picture>
        <source srcSet={productImgs.desktop} media="(min-width: 1200px)" width="1080px" height="1120px" />
        <source
          ...
          height="960px"
        />
        <source srcSet={productImgs.mobile} media="(max-width: 480px)" width="654px" height="654px" />
        <img className={styles['card__img']} src={productImgs.desktop} alt={`${productTitle}`} />
      </picture>
