...
type ElemProps = {
  ...
  productId: number;
  productImages: { desktop: string; mobile: string; tablet: string };
  productTitle: string;
};
...
function ProductExampleSeeCard(props: ElemProps): JSX.Element {
  ...
  return (
    <div className={styles['card']} aria-label={`see product ${productTitle}`}>
      <picture>
        <source srcSet={productImages.desktop} media="(min-width: 1200px)" width="700px" height="636px" />
        <source srcSet={productImages.tablet} media="(min-width: 768px)" width="446px" height="636px" />
        <source srcSet={productImages.mobile} media="(max-width: 576px)" width="654px" height="240px" />
        <img className={styles['card__img']} src={productImages.desktop} alt={productTitle} />
      </picture>
