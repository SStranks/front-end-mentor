...
function OrderCompleteCard(props: ElemProps): JSX.Element | null {
  ...
  const viewMoreBtnHandler = () => {
    grandTotalRef.current?.classList.toggle(styles['orderDetails__grandTotal--active'] as string);
    itemsListRef.current?.classList.toggle(styles['orderDetails__items--active'] as string);
  };

  return (
    <div className={styles['card']} aria-labelledby="header">
      <div className={styles['card__circle']}>
        <img className={styles['card__circle__check']} src={IconCheck} alt="checked" />
      </div>
      <h3 className={styles['card__header']} id="header">
        thank you for your order
      </h3>
      <p className={styles['card__subHeader']}>You will receive an email conformation shortly</p>
      <div className={styles['orderDetails']}>
        <div className={styles['orderDetails__items']} ref={itemsListRef}>
          {cartItemsCards}
          {cartItems.length > 1 && <hr className={styles['orderDetails__items__hr']} />}
          {cartItems.length > 1 && (
            <ViewMoreCartItemsButton cartItemLength={cartItems.length} onClickFn={viewMoreBtnHandler} />
          )}
        </div>
        <div className={styles['orderDetails__grandTotal']} ref={grandTotalRef}>
          ...
          <p className={styles['orderDetails__grandTotal__amount']}>${formatCurrency(cartTotalPrice())}</p>
        </div>
      </div>
      <button type="button" className={styles['card__homeBtn']} onClick={() => modalClose(true)}>
        back to home
      </button>
    </div>
  );
