...
function ProductDetailLayout(): JSX.Element {
  ...
  const { productCategory, productId } = location.state as LocationState;

  const product = ProductData.find((el) => el.category === productCategory && el.id === productId);
  ...
  return (
    <MainTagLayout appendClass={styles['mainTag']} id="skipto-main" testId="skipto-main">
      ...
      <ProductExampleSeeCardList appendClass={styles['productExampleSeeList']} currentProductId={productId} />
